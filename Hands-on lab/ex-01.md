...
1. On the **Create Virtual Network** blade, navigate to the **Basic** tab and enter the following information. Then, click **Next (4)** to proceed.
...
1. Go to the WGVNetRG1 Group, and select **WGVNet1** virtual network resource. Once you are on the WGVNet1 virtual network blade, select **Subnets** under **Settings** from the navigation on the left.
    ...
    ![](images/hol-ex1-task2-subnets-navigationupd.png)

1. In the **Subnets** blade select **+Subnet**.

    ![In the Subnets blade for WGVNet1, the add Subnet button is highlighted.](images/hol-ex1-task2-add-subnet-buttonupd.png "Subnets blade")
...
2. On the **Create virtual network** blade, enter the following information and then click on **IP addresses (4)** tab.

    | Setting | Action |
    | -- | -- |
    | Subscription | Keep the default subscription |
    | Resource group | Select **OnPremVNetRG (1)** |
    | Name | **OnPremVNet (2)** |
    | Region | **East US (3)** (Make sure this is **NOT** the same location you have specified in the previous exercises.) |
