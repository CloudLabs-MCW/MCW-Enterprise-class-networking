...
2. On the **Create a storage account** blade. Enter the following information, and select **Review + Create (7)** then select the **Create** button:
...
5. In the **Network Watcher** blade, select **Flow Logs (1)** under **Logs** menu and click **+ Create (2)**.

   ![](images/02032025.png)

6. In the **Create a flow log** blade that appears, enter the following information then select **Next: Analytics > (7)**.
...
7. On the **Configuration** tab of the **Create a flow log** blade, enter the following information then select **Review + create (5)**.
...
8. Repeat Steps 6 - 9 to create a flow log for the **OnPremVM-nsg** Network Security Group as well. When completed your **NSG flow logs** blade on **Network Watcher** should look like what's depicted in the below image.
...
1. Select **WGWEB1**. Connect to **WGWEB1** through **Bastion**.  In **WGWEB1**,  navigate to the load balancer's private ip address (**10.8.0.100**) and generate some traffic by refreshing the browser. Allow ten minutes to pass for traffic analytics to generate.
...
5. Once the Run diagnostic tests is complete you will get the result.
