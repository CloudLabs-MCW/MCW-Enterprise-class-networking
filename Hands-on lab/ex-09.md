## Exercise 10: Using Network Watcher to Test and Validate Connectivity

In this exercise, you will collect the flow log and perform connectivity from your simulated on-premises environment to Azure. This will be accomplished by using the Network Watcher Service in the Azure Platform.

## Lab objectives

In this lab, you will perform following tasks:

- Configuring the Storage Account for the NSG Flow Logs
- Configuring Diagnostic Logs
- Reviewing Network Traffic
- Network Connection Troubleshooting
  
## Estimated timing: 60 minutes

### Task 1: Configuring the Storage Account for the NSG Flow Logs

1. Navigate back to JupmVM, in the search bar of the Azure portal, type **Storage accounts (1)**. From the search results, select **Storage accounts (2)**.

   ![](images/lab10st7.png)

2. On the Storage account page, select **+ Create**.

3. On the **Create a storage account** blade. Enter the following information, and select **Review + Create (7)** then select the **Create** button:

    | Setting | Action |
    | -- | -- |
    | Subscription | **Select your subscription (1)** |
    | Resource group | **MonitoringRG (2)** (Use the existing resource group created earlier.) |
    | Storage Account Name | **This must be Unique and alphanumeric, lowercase, and have no special characters (3)** |
    | Region | **(US) South Central US (4)** |
    | Primary service | **Leave as Default** |
    | Performance | **Standard (5)** |
    | Replication | **Locally-redundant storage (LRS) (6)** |

    ![](images/02032025(6).png)

   >**Note:** Ensure the storage account is created before continuing.

4. Repeat steps 1 and 2, but select **East US** for the region and give it a different name.

   ![](images/e40.png)

5. In the search bar of the Azure portal, type **Network Watcher (1)**. From the search results, select **Network Watcher (2)**.

   ![](images/e38.png)

6. In the **Network Watcher** blade, select **Flow Logs (1)** under **Logs** menu and click **+ Create (2)**.

   ![](images/02032025.png)

7. In the **Create a flow log** blade that appears, enter the following information then select **Next: Analytics > (7)**.

   - Subscription: **Select your subscription (1)**

   - Flow log type: **Network Security Group (2)**

   - Click **+ Select target resource (3)** and choose **Network security group (4)**.

     ![](images/02032025(1).png)

   - On the **Select network security group** page, choose **WGAppNSG1 (1)** and click **Confirm selection (2)**.

      ![](images/02032025(2).png)

    - Storage Accounts: **The available storage account that you created earlier (1)**

    - Retention (days): **0 (2)**
    
    - Click on the **Next: Analytics (3)** tab.

      ![](images/nsg.png)

8. On the **Configuration** tab of the **Create a flow log** blade, enter the following information then select **Review + create (5)**.

    | Setting | Action |
    | -- | -- |
    | Flow Logs Version | **Version 2 (1)** |
    | Enable traffic analytics | **Checked (2)** |
    | Traffic analytics processing interval | **Every 10 mins (3)** |
    | Log Analytics Workspace | **The log analytics workspace you created earlier (4)** |

     ![](images/02032025upd.png)

9. Review the configuration and click **Create**.

     ![](images/02032025(5).png)

10. Repeat Steps 6 - 9 to create a flow log for the **OnPremVM-nsg** Network Security Group as well. Once the deployment is completed, click on **Go to resources**. When completed your **NSG flow logs** blade on **Network Watcher** should look like what's depicted in the below image.

     ![](images/hol-ex10-task1-nsg-flow-logs-blade.png)

> **Note:** The following steps are intended to generate network traffic for analysis.

11. In the search bar of the Azure portal, type **Virtual Machines (1)** in the search box and select **Virtual Machines (2)** from the results.

    ![](images/02032025(9).png)

12. On the **Virtual Machines** page, find and select **OnPremVM**.

    ![](images/02032025(10).png)

13. Click **Connect (1)** and choose **Connect (2)** as the connection method.

    ![](images/02032025(11).png)

14. Under the **Native RDP** option, click **Download RDP file**.

    ![](images/02032025(12).png)

15. If you recieve any pop up, click on **Keep**.

    ![](images/e33.png)

16. Select **Open file**.

    ![](images/e34.png)

17. Open the downloaded RDP file and click **Connect**.

    ![](images/02032025(13).png)

18. Click on **More choices**.

    ![](images/e35.png)

19. Click on **Use a different account**.

    ![](images/e36.png)    

20. When prompted, enter the following credentials and click **OK (3)**:  

    > **Note** : Please ensure to avoid spaces while copying the values from the labguide.
   
    - **Username (1):** `.\demouser`  

    - **Password (2):** `demo@pass123`

      ![](images/e37.png)

21. Click **Yes** on the security pop-up to proceed.

    ![](images/02032025(15).png)

22. On your desktop, click the **Microsoft Edge** icon to go to the Azure portal.

    - While navigating to **Microsoft edge**, if prompted select **Start without your data**

      ![](images/e41.png)

    - Click on **Confirm and continue**  

      [](images/e42.png)

    - Select **Confirm and start browsing**

      ![](images/e43.png)      
 
23. Search for [Azure portal](https://portal.azure.com).

23. You'll see the **Sign into Microsoft Azure** tab. Here, enter your credentials:
 
    - **Email/Username:** <inject key="AzureAdUserEmail"></inject>
 
24. Next, provide your password:
 
    - **Password:** <inject key="AzureAdUserPassword"></inject>
 
25. If you see the pop-up **Stay Signed in?**, click **No**.

26. In the search bar of the Azure portal, type **Virtual Machine (1)** in the search box and select **Virtual Machines (2)** from the results.

    ![](images/02032025(9).png)

27. Select **WGWEB1**. 

28. On the **Overview** pagw, select **Connect (1)** drop down and then select **Connect via Bastion (2)**. 

    ![](images/e44.png)

29. In **WGWEB1**,  Enter the following credentials and then click on **Connect (3)**.

     - **Username:** `demouser (1)`

     - **Password:** `demo@pass123 (2)`

       ![](images/e45.png)

30. If you receive an error message like the one shown in the screenshot, follow these steps:

    ![](images/lab5loadb7a.png)

31. Click on the **pop-up (1)** icon from the browser bar, select **Always allow pop-ups and redirects from https://portal.azure.com (2)**, and then click **Done (3)**.  

    ![](images/lab5loadb7b.png)

32. Now, click on **Connect** again.

    ![](images/lab5loadb7d.png)     

33. Navigate to the load balancer's private ip address (**10.8.0.100**) and generate some traffic by refreshing the browser. Allow ten minutes to pass for traffic analytics to generate.

    ![](images/hol-ex10-task1-web-traffic-over-load-balancer.png)

### Task 2: Configuring Diagnostic Logs

1. In the search bar of the Azure portal, type **Network watcher (1)**. From the search results, select **Network Watcher (2)**.

   ![](images/lab10st6.png)

2. Go to **Logs Menu**, select **Diagnostic Logs (1)**, and choose **OnPremVM-nsg (2)**.

     ![](images/02032025(7).png)

3. Click **+ Add Diagnostic Setting**.

4. Enter **OnPremDiag (1)** as the Diagnostic setting name, select **All logs (2)**, enable **Send to Log Analytics workspace (3)**, and choose the previously created workspace. 

5. Check **Archive to a storage account (4)**, select the available storage account, and click **Save (5)** to apply the settings.

     ![](images/02032025(8).png)

   > **Note** : Please repeat the above steps for adding diagnostics logs for **WGAppNSG1**.
   
   > **Note** : You can enable diagnostic logging for all network resources if required, following the same steps outlined above for now continue with below task.

### Task 3: Reviewing Network Traffic

1. In the search bar of the Azure portal, type **Network watcher (1)**. From the search results, select **Network Watcher (2)**.

   ![](images/lab10st6.png)

1. Select **Traffic Analytics** under **Monitoring** from the **Logs** menu in the blade. At this time, the diagnostic logs from the network resources have been ingested. 

     ![](images/lab10st3.png)
     
1. Select **View map**.

   ![](images/e48.png)

    >**Note:** It may take 30-40 minutes to populate. 

1. Select the **green check mark** which identifies your network. Within the pop-up menu select **More Details** to propagate detailed information of the flow to and from your network.

   ![](images/lab10st5.png)
 
   >**Note:** You can select the **See More** link to query the connections detail for more information.

### Task 4: Network Connection Troubleshooting

1. In the search bar of the Azure portal, type **Network watcher (1)**. From the search results, select **Network Watcher (2)**.

   ![](images/lab10st6.png)

2. Select **Connection Troubleshoot (2)** from the **Network Diagnostic tools (1)** menu.

   ![](images/lab10st8.png)

3. To troubleshoot a connection or to validate the route enter the following information and select **Run diagnostic tests**:

    | Setting | Action |
    | -- | -- |
    | Source Type | **Virtual Machine** |
    | Virtual Machine | **OnPremVM** |
    | Destination | **Select a virtual machine** |
    | Virtual Machine | **WGWEB1** |
    | Preferred IP version | **IPV4** |
    | Protocol | **TCP** |
    | Destination Port | **80** |
    | Diagnostic tests | **Next hop** |

   ![](images/lab10st9.png)
   
   ![](images/lab10st12.png)

4. Once the Run diagnostic tests is complete you will get the result.

    ![](images/lab10st11.png)

     >**Note**: Sometimes It may take more time to run diagnostic tests so you wont be able to see the above screenshot instantly.

### Review

In this lab, you have completed:

- Configured the Storage Account for the NSG Flow Logs
- Configured Diagnostic Logs
- Reviewed Network Traffic
- Network Connection Troubleshooting

## Great job on completing the lab!
